...
from .errors import MissionConnectionFailure, MissionTimeout


class FakeMissionGateway:
    ...
    async def connect(self, system_address: str, timeout_s: float) -> None:
        ...
        if self.failure_mode == "connection":
            raise MissionConnectionFailure(f"fake backend could not connect to {system_address}")
        self.connected = True

    async def wait_until_ready_position(self, timeout_s: float) -> PositionSample:
        await asyncio.sleep(0)
        if not self.connected:
            raise MissionConnectionFailure("fake backend is not connected")
    ...
    async def wait_until_armed(self, timeout_s: float) -> None:
        ...
        if self.failure_mode == "armed_timeout":
            raise MissionTimeout("fake backend timed out waiting for armed state")
        if not self.armed:
            raise MissionTimeout("fake backend never armed")
    ...
    async def wait_until_altitude(self, minimum_relative_altitude_m: float, timeout_s: float) -> PositionSample:
        ...
        if self.failure_mode == "takeoff_timeout":
            raise MissionTimeout("fake backend timed out during takeoff")
        return self.position
    ...
    async def goto_location(
        ...
        await asyncio.sleep(0)
        if self.failure_mode == "goto_failure":
            raise MissionTimeout("fake backend failed while issuing goto")
        self.position = replace(
            self.position,
    ...
    async def wait_until_near(
        ...
        if self.failure_mode == "waypoint_timeout":
            raise MissionTimeout("fake backend timed out while approaching waypoint")
        return self.position
    ...
    async def wait_until_landed(self, timeout_s: float) -> PositionSample:
        ...
        if self.failure_mode == "land_timeout":
            raise MissionTimeout("fake backend timed out while landing")
        return self.position

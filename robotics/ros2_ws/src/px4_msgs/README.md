...
This monorepo keeps the full `px4_msgs` source tree pinned to the PX4 release used by the project, but intentionally generates only the subset of interfaces currently exercised by `drone_px4` and the official phase validators.
...
## Generated subset in this monorepo

The workspace currently generates only:
...
The source tree remains pinned to `release/1.16`, so additional PX4 interfaces can be enabled later without changing the vendor baseline.

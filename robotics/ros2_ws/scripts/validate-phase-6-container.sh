...
start_stack() {
  if ! MICRO_XRCE_AGENT_BIN="$AGENT_BIN" MICRO_XRCE_AGENT_LD_LIBRARY_PATH="$AGENT_LD_LIBRARY_PATH" PHASE1_PYTHON_BIN=python3 PX4_SITL_MAKE_ARGS="-j$PX4_BUILD_JOBS" PHASE1_GZ_DISTRO=harmonic PHASE1_GZ_PARTITION="phase6-${ACTIVE_CASE}" PHASE1_RUNTIME_DIR="$ACTIVE_RUNTIME_DIR" PHASE1_LOG_DIR="$ACTIVE_LOG_DIR" bash scripts/sim/start.sh; then
    dump_case_logs
    return 1
  fi
